...
static int piecefreq[7];
static int specialfreq[9];
static int old_mode;
static int initial_level;
static int lines_per_level;
static int level_inc;
static int level_average;
static int special_lines;
static int special_count;
static int special_capacity;
...
static WinInfo winlist[MAXWINLIST];
static char *players[6];
static char *teams[6];
...
int xtoi(const char *buf)
{
    ...
    if (buf[0] <= '9')
        ...
        val = (toupper((int)buf[0]) - 'A' + 10) << 4;
    if (buf[1] <= '9')
        val |= buf[1] - '0';
    else
        val |= toupper((int)buf[1]) - 'A' + 10;
    return val;
}
...
{
    char buf[1024], *s, *t, *sp;
    FILE *f;
    int i;

    if (!(f = fopen("/spiffs/tetrinet.conf", "r")))
        return;
...
void write_config(void)
{
    ...
    if (!(f = fopen("/spiffs/tetrinet.conf", "w")))
        return;
...
static void add_points(int player, int points)
{
    ...
    if (!*winlist[i].name) {
        if (teams[player - 1]) {
            strncpy(winlist[i].name, teams[player - 1],
                    sizeof(winlist[i].name) - 1);
            winlist[i].name[sizeof(winlist[i].name) - 1] = 0;
            winlist[i].team = 1;
        } else {
            strncpy(winlist[i].name, players[player - 1],
                    sizeof(winlist[i].name) - 1);
            winlist[i].name[sizeof(winlist[i].name) - 1] = 0;
            winlist[i].team = 0;
        }
        ...
        if (winner > 0) {
            ...
            for (i = 1; i <= 6; i++) {
                if (teams[i - 1]) {
                    for (j = 1; j < i; j++) {
                        if (teams[j - 1]
                            && strcasecmp(teams[i - 1], teams[j - 1]) == 0)
                            break;
                    }
                    if (j < i)
                        continue;
...
static int server_parse(int player, char *buf)
{
 ...
 newplayer:
        ...
        send_to_all_but(player, "playerjoin %d %s", player,
                        players[player - 1]);
        ...
        if (total != 100) {
            send_to_all
                ("plineact 0 cannot start game: Piece frequencies do not total 100 percent!");
            return 1;
        }
        total = 0;
        for (i = 0; i < 9; i++) {
        ...
        if (total != 100) {
            send_to_all
                ("plineact 0 cannot start game: Special frequencies do not total 100 percent!");
            return 1;
        }
        playing_game = 1;
        paused = 0;
        ...
        if ((i && paused) || (!i && !paused))
            return 1;
        paused = i;
        send_to_all("pause %d", i);

    } else if (strcmp(cmd, "playerlost") == 0) {
        if (!(s = strtok_r(NULL, " ", &sp)) || atoi(s) != player)
            return 1;
        player_loses(player);
...
static int init()
{
 ...
 ipv4_success:
    ...
    if (listen_sock6 >= 0) {
        i = 1;
        if (setsockopt(listen_sock6, SOL_SOCKET, SO_REUSEADDR, &i, sizeof(i)) ==
            0) {
...
static void find_hash_pattern(unsigned char *buf, size_t buflen, bool tetrifast, unsigned char *hash, size_t *hashlen)
{
	char *cmd = tetrifast ? "tetrifaster" : "tetrisstart";
	size_t cmdlen = strlen(cmd);

	if (buflen < cmdlen + 1 || *hashlen < cmdlen) {
		*hashlen = 0;
		return;
	}

	for (int i = 0; i < cmdlen; i++) {
		hash[i] = ((cmd[i] + buf[i]) % 255) ^ buf[i + 1];
	}

	*hashlen = 5;
	for (int i = 5; i == *hashlen && i > 0; i--) {
		for (int j = 0; j < cmdlen - *hashlen; j++) {
			if (hash[j] != hash[j + *hashlen]) {
				*hashlen -= 1;
			}
		}
	}
...
static int decode_reg(unsigned char *buf, size_t *buflen)
{
	if (*buflen % 2 != 0) {
		return 1;
	}
	...
	*buflen /= 2;
	for (int i = 0; i < *buflen; i++) {
		buf[i] = (char)xtoi((char *)buf + i*2);
	}

	unsigned char hash[11];
	size_t hashlen = sizeof(hash);
	find_hash_pattern(buf, *buflen * 2, false, hash, &hashlen);
	if (hashlen == 0) {
		hashlen = sizeof(hash);
		find_hash_pattern(buf, *buflen * 2, true, hash, &hashlen);
		if (hashlen == 0) {
			return 1;
		}
	}
	...
	for (int i = 0; i < *buflen - 1; i++) {
		buf[i] = (((buf[i + 1] ^ (hash[i % hashlen])) + 255 - buf[i]) % 255);
	}
	buf[*buflen - 1] = '\0';
...
static void check_sockets()
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (player_socks[i] < 0) {
            ...
            if (decode_reg((unsigned char *)buf, &len)) {
                close(fd);
                player_socks[i] = -1;
                continue;
            }
