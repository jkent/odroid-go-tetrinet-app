    ...
    if (piece_overlaps(-1, -1, -1)) {
        ...
        if (piece_overlaps(-1, -1, -1)) {
            ...
            if (piece_overlaps(-1, -1, -1)) {
                ...
                send_field(NULL);
                sockprintf(sock, "playerlost %d", my_playernum);
                playing = 0;
                spectating = 1;
            }
        }
...
void step_down(void)
{
    ...
    if (y + 1 + pd->bottom < FIELD_HEIGHT && !piece_overlaps(-1, ynew, -1)) {
        ...
        if (old_mode && completed > 1) {
            if (completed < 4)
                completed--;
            sockprintf(sock, "sb 0 cs%d %d", completed, my_playernum);
        ...
        if (completed > 0) {
            sockprintf(sock, "lvl %d %d", my_playernum, level);
            io->draw_status();
        }
...
void do_special(const char *type, int from, int to)
{
    ...
    if (!playing)
        return;
    if (to != 0 && to != my_playernum
        && !(from == my_playernum && *type == 's'))
        return;
